...
class DataGenerator(Dataset):
    def __init__(self, cover_dir, stego_dir):
        ...
        img = imageio.v2.imread(join(self.cover_path, self.filename_list[0]))
    ...
    def __getitem__(self, index):
        ...
        batch[0, :, :, :] = imageio.v2.imread(join(self.cover_path, self.filename_list[index]))
        batch[1, :, :, :] = imageio.v2.imread(join(self.stego_path, self.filename_list[index]))
...
def generate_data(data_path, batch_size):
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=batch_size['train'],
